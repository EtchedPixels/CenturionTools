...
#define BINPATH		"/opt/aswrx6/bin/"
#define LIBPATH		"/opt/aswrx6/lib/"
#define INCPATH		"/opt/aswrx6/include/"

#define CMD_AS		BINPATH"aswrx6"
#define CMD_CC		LIBPATH"ccwrx"
#define CMD_COPT 	LIBPATH"copt"
#define COPT_FILE 	LIBPATH"ccwrx.rules"
#define CMD_LD		BINPATH"ldwrx6"
#define CRT0		LIBPATH"crt0.o"
#define LIBC		LIBPATH"libc.a"
#define LIBCPU6		LIBPATH"libwrx6.a"
#define LIBC		LIBPATH"libc.a"
...
/* This turns -L/opt/aswrx/lib  -lfoo -lbar into resolved names like
   /opt/aswrx/lib/libfoo.a */
...
/*	add_argument("-r"); No register support yet */
	add_argument("--add-source");
	add_argument("--cpu");
	switch(cpu) {
		case 4:
			add_argument("4");
			add_argument("-D__CPU4__");
			break;
		case 6:
			add_argument("6");
			add_argument("-D__CPU6__");
			break;
	}
	switch(targetos) {
	case OS_FUZIX:	/* Fuzix */
...
/*
	if (cpu == 4)
		append_obj(&liblist, LIBCPU4, TYPE_A);
	else */if (cpu == 6)
		append_obj(&liblist, LIBCPU6, TYPE_A);
...
void usage(void)
{
	fprintf(stderr, "ccwrx6: unknown arguments.\n");
	fatal();
}
...
/*                    split_id();*/
		...
		case 'm':
			cpu = atoi(*p + 2);
			if (cpu != 4 && cpu != 6) {
				fprintf(stderr, "cc: only 4 or 6 supported.\n");
				fatal();
			}
			break;	
		case 'M':
